...
library(data.table)
gProfiler_data <- fread('gProfiler_.csv', data.table = FALSE)
...
count_num_parents <- function(go_term) {
  ...
  if (p[['is_a']] == 'all') {
    ...
    return(1 + count_num_parents(p[['is_a']]))
  }
}
...
p1 <- p1 + scale_size( range=c(1, 20), breaks = c(500, 1000)) + theme_bw(); # + scale_fill_gradientn(colours = heat_hcl(7), limits = c(-300, 0) );
...
library(ggrepel)
p1 <- p1 + geom_label_repel(data = ex, aes(plot_X, plot_Y, label = description, colour = log10_p_value), size = 2 , force =20, fontface = "bold");
...
p1 <- p1 + guides(size = guide_legend(override.aes = list(fill = '#ffffff', alpha = 1, order = 2), title = 'Size of intersection'), colour = guide_colourbar(title = 'Log10 (p value)', order = 1))


# --------------------------------------------------------------------------
# Output the plot to screen

svg("revigo_plot_multiclass.svg", width = 7, height = 4)
p1;
dev.off()
